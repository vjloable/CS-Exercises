...
{
    BinNode<T> *hopper = root;
    int isIncluded = 0;
    while(hopper != NULL){
        if (val == hopper->element){
            isIncluded = 1;
            break;
        }
        hopper = (val <= hopper->element) ? hopper->left : hopper->right;
    }
    return isIncluded;
}
...
{
    BinNode<T> *cursor = this->root;
    BinNode<T> *anchor = NULL;

    while(cursor != NULL){
        anchor = cursor;
        cursor = (val <= cursor->element) ? cursor->left : cursor->right;
    }

    BinNode<T> *value = makeBinNode<T>(val);
    if (anchor == NULL){
        this->root = value;
    }else if (val <= anchor->element){
        anchor->left = value;
    }else{
        anchor->right = value;
    }
...
template <class T> T BSTree<T>::remove(T &val)
{
    ...
    while(1){
        if (val == cursor->element || cursor == NULL){
            break;
        }
        anchor = cursor;
        cursor = (val <= cursor->element) ? cursor->left : cursor->right;
    ...
    BinNode<T> *valueTop = removeTop(cursor);
    if (anchor == NULL){
        root = valueTop;
    }else if (val <= anchor->element){
        anchor->left = valueTop;
    }else{
        anchor->right = valueTop;
    }
...
template <class T> void levelorder(BinNode<T> *root){
    if (root != NULL){
        ...
        while (1){
            ...
            temp.pop();
            if (cursor->left){
                temp.push(cursor->left);
            }
            if (cursor->right){
                temp.push(cursor->right);
            }
            if (temp.empty())
            {
                break;
